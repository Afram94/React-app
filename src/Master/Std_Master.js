  ...
  return (
    <div>
      ...
      <div>
        ...
        {/* Page name */}
        <div className='absolute left-22 -top-20 z-0 w-full justify-center text-center text-xl md:text-4xl lg:text-5xl xl:text-6xl text-white font-bold' style={{marginTop: "12vw"}}> {/* right-[10vw] md:right-[15vw] lg:right-[150px] xl:right-[250px] */}
          {props.Title}
        </div>

        {/* <img src={Smart_cash_gradientBg} className='w-full' resizeMode="repeat"/> */}
        <div className='w-full h-[400px] bg-black bg-cover bg-no-repeat inline -skew-y-6' >
          <img src={bild_1} className='w-full h-full' />
        </div>
      </div>
      <div>
        {props.inner}
    ...
    <div className="z-10">
      <div className={'flex flex-cols px-4 col-span-1 w-full justify-center ' + (isSm ? "hidden" : "")} style={{marginRight: "7%"}}>
        ...
        <div className="flex flex-col">
          <div className="text-white mx-3 font-bold uppercase cursor-pointer text-lg" onClick={() => {setToggle_products(!toggle_products); handleClick();}}>Produkter</div>
          ...
          {/* <animated.div style={productAnimation}> */}
            <div className="absolute top-[40px] right-[-10px]">
              <div className={"product_menu flex bg-white rounded-xl shadow-md h-auto w-[940px] my-4 py-2 " + ((toggle_products) ? "block " : "hidden ") + css(styles.slideInRight)}>
                ...
                <Link to="/beauty-products" className='text-black mx-6 font-bold uppercase flex justify-self-center'> <div className="w-6 h-6 mr-1 pt-1"><MdOutlineVolunteerActivism/> </div>       Skönhet & Hälsa     </Link>
        ...
        <div className="flex flex-col">
          <div className="text-white mx-3 font-bold uppercase cursor-pointer text-lg" onClick={() => {setToggle_offer(!toggle_offer); setShowOfferAnimation(!showOfferAnimation);}}>Erbjudande</div>
          {/* <animated.div style={offerAnimation}> */}
            <div className="mr-24 absolute top-[40px] right-[700px]">
              <div className={"offer_menu flex bg-white rounded-xl shadow-md h-auto w-[700px] absolute my-4 py-2 " + ((toggle_offer) ? "block " : "hidden ") + css(styles.slideInRight)}>
                <Link to="/restaurant-and-cafe" className='text-black mx-8 font-bold uppercase'>       Restaurang & Cafe    </Link>
                <Link to="/beauty-and-health" className='text-black mx-8 font-bold uppercase'>       Skönhet & Hälsa    </Link>
        ...
        <Link to="/inlosenavtal" className='text-white mx-3 font-bold uppercase text-lg'>       Inlösenavtal     </Link>
        <Link to="/om_oss" className='text-white mx-3 font-bold uppercase w-20 text-lg'>      Om  oss        </Link>
        <Link to="/kontakt" className='text-white mx-3 font-bold uppercase text-lg'>       Kontakt       </Link>
      ...
      <div className={'w-screen bg-white absolute h-screen z-10 ' + (isSm ? "" : "hidden")}>
        <div className={'grid-rows-5 grid px-4 row-span-1 w-full justify-center items-center self-center py-14 '} style={{marginRight: "7%"}}>
        <div className="flex flex-col">
          <div className="text-black mx-8 font-bold uppercase cursor-pointer flex justify-center" onClick={() => setToggle_products(!toggle_products)}>Produkter</div>
          <div className={" bg-white rounded-xl shadow-md h-auto w-auto " + ((toggle_products) ? "block" : "hidden")}>
            <Link to="/" className='text-black mx-8 font-bold uppercase flex justify-self-center py-2'>        Restaurang & Cafe     </Link>
            <Link to="/" className='text-black mx-8 font-bold uppercase flex justify-self-center py-2'>        Restaurang & Cafe     </Link>
            <Link to="/" className='text-black mx-8 font-bold uppercase flex justify-self-center py-2'>        Restaurang & Cafe     </Link>
            <Link to="/" className='text-black mx-8 font-bold uppercase flex justify-self-center py-2'>        Restaurang & Cafe     </Link>
          </div>
